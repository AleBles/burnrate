...
describe('isProxiedPath: path matching rule', () => {
  ...
  // Path normalization only lowercases on darwin/win32 (case-insensitive
  // filesystems); on case-sensitive Linux a mixed-case mismatch is correct
  // behavior, so this assertion only holds on those platforms.
  it.skipIf(process.platform !== 'darwin' && process.platform !== 'win32')(
    'is case-insensitive (macOS/Windows default filesystems)', () => {
    setProxyPaths(['/Users/Me/Work'])
    expect(isProxiedPath('/users/me/work/acme')).toBe(true)
  })

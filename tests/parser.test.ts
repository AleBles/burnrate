...
beforeEach(async () => {
  ...
  process.env['BURNRATE_CACHE_DIR'] = tmpCache
...
describe('(a) copilot JSONL file-purge monotonic', () => {
  it('preserves monthly total after events.jsonl is deleted', async () => {
    ...
    vi.stubEnv('BURNRATE_COPILOT_SESSION_STATE_DIR', sessionStateDir)
    vi.stubEnv('BURNRATE_COPILOT_DISABLE_OTEL', '1')
    vi.stubEnv('BURNRATE_COPILOT_WS_STORAGE_DIR', join(tmpHome, 'no-ws'))
...
describe.skipIf(!isSqliteAvailable())(
  ...
  () => {
    it('preserves total after one conversation is pruned from the OTel DB', async () => {
      ...
      vi.stubEnv('BURNRATE_COPILOT_OTEL_DB', dbPath)
      vi.stubEnv('BURNRATE_COPILOT_DISABLE_OTEL', '')
      vi.stubEnv('BURNRATE_COPILOT_SESSION_STATE_DIR', join(tmpHome, 'no-jsonl'))
      vi.stubEnv('BURNRATE_COPILOT_WS_STORAGE_DIR',   join(tmpHome, 'no-ws'))

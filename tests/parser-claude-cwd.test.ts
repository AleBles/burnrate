...
beforeEach(async () => {
  ...
  process.env['BURNRATE_DESKTOP_SESSIONS_DIR'] = join(tmpDir, 'desktop-sessions')
})
...
describe('Claude cwd project paths', () => {
  ...
  it('groups git worktrees under the main repository project', async () => {
    const mainRepo = join(tmpDir, 'repos', 'burnrate')
    ...
    expect(projects).toHaveLength(1)
    expect(projects[0]!.project).toBe('burnrate')

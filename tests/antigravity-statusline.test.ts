...
describe('Antigravity CLI statusLine hook installer', () => {
  async function withTempSettings(run: (dir: string, settingsPath: string) => Promise<void>) {
    ...
    const codeburnPath = join(binDir, process.platform === 'win32' ? 'burnrate.cmd' : 'burnrate')
    await mkdir(binDir, { recursive: true })
    await writeFile(codeburnPath, process.platform === 'win32' ? '@echo off\r\n' : '#!/bin/sh\n')
    await chmod(codeburnPath, 0o755)
    process.env['BURNRATE_ANTIGRAVITY_SETTINGS_PATH'] = settingsPath
    process.env['BURNRATE_CACHE_DIR'] = join(dir, 'cache')
  ...
  it('skips transient npx codeburn shims when resolving the hook command', async () => {
    await withTempSettings(async (dir) => {
      ...
      const npxCodeburn = join(npxBin, process.platform === 'win32' ? 'burnrate.cmd' : 'burnrate')
      const persistentCodeburn = join(persistentBin, process.platform === 'win32' ? 'burnrate.cmd' : 'burnrate')

...
describe('antigravity provider helpers', () => {
  ...
  it('captures exact Antigravity CLI statusLine usage as fallback calls', async () => {
    ...
    process.env['BURNRATE_CACHE_DIR'] = dir
  ...
  it('parses current Antigravity CLI SQLite conversations with non-zero token usage', async () => {
    ...
    const previousCacheDir = process.env['BURNRATE_CACHE_DIR']
    process.env['BURNRATE_CACHE_DIR'] = cacheDir
    ...
    try {
      ...
      if (previousCacheDir === undefined) delete process.env['BURNRATE_CACHE_DIR']
      else process.env['BURNRATE_CACHE_DIR'] = previousCacheDir
      await rm(tempHome, { recursive: true, force: true })
    }
  })

...
beforeEach(async () => {
  ...
  originalCodeburnCacheDir = process.env['BURNRATE_CACHE_DIR']
})

afterEach(async () => {
  if (originalHermesHome === undefined) delete process.env['HERMES_HOME']
  else process.env['HERMES_HOME'] = originalHermesHome
  if (originalCodeburnCacheDir === undefined) delete process.env['BURNRATE_CACHE_DIR']
  else process.env['BURNRATE_CACHE_DIR'] = originalCodeburnCacheDir
...
async function loadParserWithHermesHome(hermesHome: string, codeburnCacheDir: string) {
  ...
  process.env['BURNRATE_CACHE_DIR'] = codeburnCacheDir
  vi.resetModules()

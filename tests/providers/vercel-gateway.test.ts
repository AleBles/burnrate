...
describe('vercel-gateway end-to-end (parseAllSessions network path)', () => {
  const originalFetch = globalThis.fetch
  const originalKey = process.env.AI_GATEWAY_API_KEY
  const originalCacheDir = process.env.BURNRATE_CACHE_DIR
  let cacheDir: string
  ...
  afterEach(async () => {
    ...
    if (originalCacheDir === undefined) delete process.env.BURNRATE_CACHE_DIR
    else process.env.BURNRATE_CACHE_DIR = originalCacheDir
    clearSessionCache()
    vi.restoreAllMocks()

...
// Local-date → "YYYY-MM-DD". Inlined from the removed daily-cache module
// (burnrate dropped the daily cache); this was its only surviving consumer.
...
export function toPeriod(s: string): Period {
  ...
  process.stderr.write(
    `burnrate: unknown period "${s}". Valid values: ${VALID_PERIODS.join(', ')}.\n`
  )
  process.exit(1)
}
...
export function getDateRange(period: string): { range: DateRange; label: string } {
  ...
  switch (period) {
    ...
    default: {
      process.stderr.write(
        `burnrate: unknown period "${period}". Valid values: today, week, 30days, month, all.\n`
      )
      process.exit(1)
    }
  }

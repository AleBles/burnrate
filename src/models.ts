...
function getCacheDir(): string {
  if (process.env['BURNRATE_CACHE_DIR']) return process.env['BURNRATE_CACHE_DIR']
  return join(homedir(), '.cache', 'burnrate')
}
...
// Test seam: reset pricing back to the bundled snapshot, discarding any pricing
// loaded from the live LiteLLM cache by loadPricing(). pricingCache is
// module-level state shared across the whole test run, so a test that asserts
// snapshot/fallback behavior must reset it to stay deterministic.
...
function shouldWarnAboutUnknownModel(name: string): boolean {
  ...
  if (process.env['BURNRATE_VERBOSE'] !== '1') return false
  return true
}
  ...
  if (!costs) {
    if (shouldWarnAboutUnknownModel(model)) {
      ...
      process.stderr.write(
        `burnrate: no pricing data for model "${safeName}" — costs for this model will show $0. ` +

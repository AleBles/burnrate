...
async function discoverFromDb(dbPath: string, profile: string): Promise<SessionSource[]> {
  ...
  try {
    ...
    if (isSqliteBusyError(err)) throw err
    process.stderr.write(`burnrate: error querying Hermes database: ${err instanceof Error ? err.message : err}\n`)
    return []
  } finally {
    db.close()
  }
  ...
  return {
    async *parse(): AsyncGenerator<ParsedProviderCall> {
      ...
      try {
        ...
        process.stderr.write(`burnrate: cannot open Hermes database: ${err instanceof Error ? err.message : err}\n`)
        return
      }

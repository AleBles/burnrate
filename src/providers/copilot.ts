...
//   BURNRATE_COPILOT_OTEL_DB    — Override the agent-traces.db path
//   BURNRATE_COPILOT_DISABLE_OTEL=1 — Skip OTel entirely, use only JSONL
//
// ARCHITECTURE:
...
function getCopilotSessionStateDir(override?: string): string {
  return override ?? process.env['BURNRATE_COPILOT_SESSION_STATE_DIR'] ?? join(homedir(), '.copilot', 'session-state')
}
 ...
 *
 * Priority:
 *   1. BURNRATE_COPILOT_OTEL_DB env var
...
function getAgentTracesDbPath(): string | null {
  ...
  const envOverride = process.env['BURNRATE_COPILOT_OTEL_DB']
  if (envOverride) {
...
export function createCopilotProvider(sessionStateDir?: string, workspaceStorageDir?: string): Provider {
   ...
   * directory is used. The BURNRATE_COPILOT_WS_STORAGE_DIR env var provides a
  ...
  function getWsDirs(): string[] {
    ...
    const envDir = process.env['BURNRATE_COPILOT_WS_STORAGE_DIR']
  ...
  return {
    ...
    async discoverSessions(): Promise<SessionSource[]> {
      ...
      const disableOtel = process.env['BURNRATE_COPILOT_DISABLE_OTEL'] === '1'
      if (!disableOtel) {

...
function parseBubbles(
  ...
  // Override the budget in tests via BURNRATE_CURSOR_MAX_BUBBLES.
  const MAX_BUBBLES = Number(process.env['BURNRATE_CURSOR_MAX_BUBBLES']) || 250_000
    ...
    if (total > MAX_BUBBLES) {
      ...
      if (scan.truncated) {
        process.stderr.write(
          `burnrate: Cursor database has ${total.toLocaleString()} bubbles and the ` +
  ...
  if (skipped > 0) {
    process.stderr.write(`burnrate: skipped ${skipped} unreadable Cursor entries\n`)
  }
    ...
    async *parse(): AsyncGenerator<ParsedProviderCall> {
      ...
      if (cached) {
        ...
        try {
          ...
          process.stderr.write(`burnrate: cannot open Cursor database: ${err instanceof Error ? err.message : err}\n`)
          return
        }
        try {
          if (!validateSchema(db)) {
            process.stderr.write('burnrate: Cursor storage format not recognized. You may need to update burnrate.\n')
            return
          }

...
function warnUnrecognizedSchemaOnce(providerLabel: string, missing: string[]): void {
  ...
  process.stderr.write(
    `burnrate: ${providerLabel} database is missing expected tables (${missing.join(', ')}). ` +
  ...
  return {
    async *parse(): AsyncGenerator<ParsedProviderCall> {
      ...
      try {
        ...
        process.stderr.write(`burnrate: cannot open ${config.displayName} database: ${err instanceof Error ? err.message : err}\n`)
        return
      }
      ...
      try {
        ...
        if (yieldCount === 0 && messages.length > 0) {
          ...
          if (yieldCount === 0 && process.env['BURNRATE_VERBOSE'] === '1') {
            process.stderr.write(
              `burnrate: ${config.displayName} session ${sessionId} has ${messages.length} messages ` +

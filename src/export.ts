...
/// burnrate export without ever deleting a user's unrelated files by accident.
const EXPORT_MARKER_FILE = '.burnrate-export'

async function isBurnRateExportFolder(path: string): Promise<boolean> {
  const markerStat = await stat(join(path, EXPORT_MARKER_FILE)).catch(() => null)
  return markerStat?.isFile() ?? false
}

async function clearBurnRateExportFolder(path: string): Promise<void> {
...
/// pre-existing file or a non-burnrate folder, so a typo like `-o ~/.ssh/id_ed25519` can't

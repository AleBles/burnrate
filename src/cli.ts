...
program
  ...
  .action(async (opts) => {
    ...
    if (opts.from || opts.to) {
      ...
      if (!customRange) {
        process.stderr.write('burnrate: --from and --to must be valid YYYY-MM-DD dates\n')
        process.exit(1)
      }
      range = customRange
    } else {
    ...
    if (fmt === 'json') {
      ...
      process.stderr.write(`burnrate: unknown --format "${opts.format}". Choose table, markdown, json, or csv.\n`)
      process.exit(1)
    }
  })
